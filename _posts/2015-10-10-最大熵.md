...
$$\begin{split}
L(P,w) &= -H(Y|X) + w_0(1 - \sum_y{p(y|x)}) + \sum_{i=1}^{m}{w_i \left (\sum_{x,y}{E_{\tilde p} \big (f_i(x,y) \big ) - E_{p} \big({f_i(x,y) \big)}} \right) } \\
...
$$\begin{equation} \label{modelexp}
p(y|x) = e^{\sum_{i=1}^{m}{w_if_i(x,y)} + w_0 - 1} = \frac{e^{\sum_{i=1}^{m}{w_if_i(x,y)}}{e^{1 - w_0}}
\end{equation}$$

我们把$e^{1 - w\_0}$先定义为$Z_w(x)$，则($\ref{modelexp}$)可以表示为

$$\begin{equation} \label{modelexpnormal}
...
$Z\_x(w)$被称为归一化因子（或者规范化因子），保证$\sum\_y$和为1；$f\_i(x,y)$是第$i$个特征函数，$w\_i$是对应该特征函数的权值。
