...
$$\begin{equation} \label{maxentropytmp}
p(y|x) = e^{\sum_{i=1}^{m}{w_if_i(x,y)} + w_0 - 1} = \frac{e^{\sum_{i=1}^{m}{w_if_i(x,y)}}{e^{1 - w_0}}
\end{equation}$$

我们把$e^{1 - w\_0}$先定义为$Z_w(x)$，则($\ref{maxentropytmp}$)可以表示为
...
$$\begin{split}
\Psi(w) &= &\sum_{x,y} \tilde p(x) p(y|x) \log p(y|x) + w_0 \left ( 1 - \sum_y{p(y|x)}\right ) \\
& &+ \sum_{i=1}^{m}{w_i \left(\sum_{x,y}\tilde p(x,y) f_i(x,y) - \sum_{x,y}\tilde p(x) p(y|x) f_i(x,y) \right)}
\end{split}$$
...
$$\begin{split}
\Psi(w) &= &\sum_{x,y} \tilde p(x) p(y|x) \log p(y|x) +  \sum_{i=1}^{m}{w_i \left(\sum_{x,y}\tilde p(x,y) f_i(x,y) - \sum_{i=1}^m \tilde p(x) p(y|x) f_i(x,y) \right)}\\
(带入p(y|x) = \frac{1}{Z_w(x)} e^{\sum_{x,y}x_i f_i(x,y)}) \Rightarrow \\
&= &\sum_{x,y} \tilde p(x) \frac{1}{Z_w(x)} e^{\sum_{i=1}^{m}x_i f_i(x,y)} \left ( \sum_{i=1}^m w_i f_i(x,y) - \log Z_w(x) \right ) \\
& &+\sum_{i=1}^{m}w_i\left( \sum_{x,y}\tilde p(x,y) f_i(x,y) - \sum_{x,y} \tilde p(x) \frac{1}{Z_w(x)}e^{\sum_{i=1}^m w_i f_i(x,y)}f_i(x,y) \right ) \\
(消去复杂的两项) \Rightarrow \\
&= &\sum_{x,y} \tilde p(x,y) \sum_{i=1}^{m} w_i f_i(x,y) - \sum_{x,y} \tilde p(x) \frac{1}{Z_w(x)}e^{\sum_{i=1}^m w_i f_i(x,y)} \log Z_w(x) \\
\end{split}$$

根据 $\sum\_y{p(y\|x)} = 1 $ ， 对$\sum\_{x,y} \tilde p(x) \frac{1}{Z\_w(x)}e^{\sum\_{i=1}^m w\_i f\_i(x,y)} \log Z\_w(x) $对$y$求和式，由于$\tilde p(x) $, $\log Z\_w(x)$与$y$无关，故$\sum\_y{\frac{1}{Z\_w(x)}e^{\sum\_{i=1}^m w\_i f\_i(x,y)}} = \sum_y p(y\|x) = 1 $ ，有： 

$$\begin{equation} \label{psifunc}
\Psi(w) = \sum_{x,y} \tilde p(x,y) \sum_{i=1}^{m} w_i f_i(x,y) - \sum_x \tilde p(x) \log Z_w(x)
\end{equation}$$

要是$\Psi(w)$最大，与上面思路相同——对$w$求导，令其等于0即可，即
...
2. 求对偶问题内部的极小问题。得到最大熵模型的表示$P_w(y\|x)$。
    ...
    2. 求对偶问题的解。由于是求最小，就是对其求导，令导数为0。求得对应的$P$的表示($P\_w = \arg_\p min \Psi(w)$)。
...
根据后续理解，在实际使用中，我们只需要记住最大熵模型的表示形式，即$p(y\|x) = \frac{1}{Z\_w(x)} e^(\sum\_{i=1}^m w\_i f\_i(x,y))$。然后带入到$\Psi(w)$中，使用IIS或者拟牛顿法迭代求解即可。
