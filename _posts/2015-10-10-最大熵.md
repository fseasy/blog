...
& &+  \sum_{i=1}^{m}{w_i \left(\sum_{x,y}\tilde p(x,y) f_i(x,y) - \sum_{i=1}^m \tilde p(x) p(y|x) f_i(x,y) \right)}
\end{split}$$

带入$ p(y\|x) = \frac{1}{Z\_w(x)} e^{\sum\_{x,y}x\_i f\_i(x,y)}) $ 

$$\begin{split}
\Psi(w) &= &\sum_{x,y} \tilde p(x) \frac{1}{Z_w(x)} e^{\sum_{i=1}^{m}x_i f_i(x,y)} \left ( \sum_{i=1}^m w_i f_i(x,y) - \log Z_w(x) \right ) \\
...
由于$\tilde p(x) $, $\log Z\_w(x)$与$y$无关，故$\sum\_y{\frac{1}{Z\_w(x)}e^{\sum\_{i=1}^m w\_i f\_i(x,y)}} = \sum_y p(y\|x) = 1 $ ，有： 
