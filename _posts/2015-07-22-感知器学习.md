...
&= \frac{n\cdotW_0 + n\cdot\Delta_1 + (n-1)\cdot\Delta_2 + \dots + \Delta_n}{n} \\
...
$$\begin{align*}
sum_val &= \underbrace{origin_sum_val}_{原始和} + \underbrace{(current_time - last_update_time -1 ) \cdot val}_{过去时间应该加上的值} + \underbrace{( val + delta )}_{本次的权值} 
&= origin_sum_val + (current_time - last_update_time) \cdot val + \Delta
\end{align*}$$
