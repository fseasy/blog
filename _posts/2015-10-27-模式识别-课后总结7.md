...
$$\begin{equation}
\label{ogradient} \frac{\partial J(W)}{\partial w_ij} = \delta_i y_j
\end{equation}$$
...
$$\begin{equation}
\label{ogradient} \frac{\partial J(W)}{\partial w_{jk}} = \delta_j x_k
\end{equation}$$

由$(\ref{hSensitivity})(\ref{\ogradient})$，就可以表示隐含层每个节点中每个权值的梯度了。
