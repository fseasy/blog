...
> 完整的 FAST 算法，可以参见 [Fast OpenCV Tutorial][fast_opencv_tutorial]. OpenCV 实现的 FAST, 并没有完全遵循原论文， 而是抛弃了其中的用 ID3 计算最优筛选顺序的逻辑，直接用按规则来选定比较顺序（FAST 原论文就叫 *Machine learning for high-speed corner detection*, 这下直接把 ML 部分给去了 :dog:）. OpenCV FAST 源码解析，有兴趣可看[OpenCV FAST 角点检测算法 CPU 版本实现源码注解]({% post_url 2022-08-17-opencv_f2d_fast %})
