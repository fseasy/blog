...
1. 旋转矩阵是正交矩阵
   ...
   所谓正交矩阵，就是满足下面[任意条件][orthogonal_define]的矩阵：

   1. $A^T A = I$
   2. $A^T = A^{-1}$, 转置等于逆
   3. $A$ 的每行都是正交的单位向量，或者每列都是正交的单位向量

   这几个都是等价的（第一、第二点比较显然。第三点我没有想过怎么推导）。我们可以从第二点出发去验证上面的 $R^T = R^{-1}$.

   求解 $R^T$ 是很简单的；求解 $R^{-1}$, 最简单是回到 \ref{ref1}: 
   之前 $R$ 表示从 $(\bs{e_1'}, \bs{e_2'}, \bs{e_3'})$ 转到  $(\bs{e_1}, \bs{e_2}, \bs{e_3})$, 现在我们反过来，
   从  $(\bs{e_1}, \bs{e_2}, \bs{e_3})$ 转到  $(\bs{e_1'}, \bs{e_2'}, \bs{e_3'})$， 即在 \ref{ref1} 中左乘
   $\begin{bmatrix}\bs{e_1'^T}, \bs{e_2'^T}, \bs{e_3'^T}\end{bmatrix}^T$，得到的 $R'$ 就是 $R$ 的逆了。
   可以在纸上写下，$R'$ 和 $R^T$ 值是一样的，因此旋转矩阵时正交矩阵。

2. 旋转矩阵行列式为1
  
   正交矩阵的行列式可以是 1 或 -1. 为 1 时就是旋转矩阵，为 -1 时被称为瑕旋转。
