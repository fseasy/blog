...
双目相机模型是利用*视差*原理来还原物体的深度的，如下图所示：
...
\frac{Z_c - f}{Z_c} &= \frac{\overline{O_LO_R} - \overline{P_{fL}P_{fR}}}{\overline{O_LO_R}} \\
                    &= \frac{b - (\vert X_{fL} \vert + \vert X_{fR} \vert)}{b}
                    \qquad \scriptstyle{\text{//}X_{fL}\text{为正，}X_{fR}\text{为负}}\\
