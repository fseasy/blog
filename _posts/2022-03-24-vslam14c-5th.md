...
> 该图基于 [opencv-calibration][opencv_cali] 中[针孔相机模型图][pinhole_model]，参考书上的符号/过程绘制。
...
3. 计算 $P_f = (X_f, Y_f, f)$ 转换到像素平面的坐标 $p = (u, v)$
   ...
   - $P_f$ 坐标的单位是 `mm` 等距离单位，而 $p$ 的单位是像素。前者转换到后者，需要采样和缩放。
     在数学上，就是要乘上一个单位类似 `pixel/mm` 的缩放系数。我们常说的 PPI (Pixels Per Inch) 应该就属于这种缩放系数。
   
   - 一般地，X轴和Y轴的缩放系数是不同的，这里分别记为 $\alpha$, $\beta$. 而 $P_f$ 的Z轴信息，在平面上直接丢掉了。

     > 为何缩放系数不同，[下面](#为什么转换到像素坐标系时x轴y轴缩放比例一般不同)有简单的猜测。
   
   - 像素坐标系的坐标原点 $o$ 在平面左上角，$u$ 轴平行同向于相机坐标系 $X$ 轴， $v$ 轴平行同向于 $Y$ 轴。
     相机坐标系原点 $O$ 沿光轴方向（即 $Z$ 轴）落在像素平面的点被叫做主点[^2]。 记主点在像素平面的坐标为 $c = (c_x, c_y)$.
     
     一般主点就在像素平面的中心(所以也叫 image center)。但也并非总是如此（至少有工程安装难以绝对精准的原因）——故 $c$ 也是一个需要求解的量！

     [^2]: 见 [The_geometry_and_mathematics_of_the_pinhole_camera][pinehole_geometry]，是像素平面上的点。 需要区别于[Principal_planes_and_points][principal_point_optic]中透镜上的 principal point.

    <div align="center">
      <img src="/assets/posts/vslam/14-ch5/to_image_plane.png" alt="focal plane to image plane"
          width="300">
      <p>图2：对称成像平面的点落到像素平面</p>
    </div>

    基于上面的背景信息，绘制上图。对称成像平面的原点 $O_f$ 与像素平面主点 $c$ 对应，$P_f$ 横、纵坐标经 $\alpha$, $\beta$ 缩放后落到 $p$ 点， 
    则 $\vec{cp} = (\alpha X_f, \beta Y_f)$. 又 $\vec{oc} = (c_x, c_y)$, 
    则 $\vec{op} = \vec{oc} + \vec{cp} = (\alpha X_f + c_x, \beta Y_f + c_y)$, 而这也即 $p$ 在像素平面的坐标。
    写作矩阵形式，为：$p = \begin{bmatrix}\alpha & 0 \\\\ 0 & \beta \end{bmatrix} \begin{bmatrix} X_f \\\\ Y_f \end{bmatrix} + \begin{bmatrix}c_x \\\\ c_y\end{bmatrix} $. 

将2、3步骤合起来，则从相机坐标系下的 $P_c$ 到像素坐标系的 $p$, 公式为： 
$p = \begin{bmatrix}\alpha \frac{f}{Z_c} & 0 \\\\ 0 & \beta \frac{f}{Z_c} \end{bmatrix} \begin{bmatrix} X_c \\\\ Y_c \end{bmatrix} + \begin{bmatrix}c_x \\\\ c_y\end{bmatrix} $. 令 $f_x = \alpha f$, $f_y = \beta f$, 且引入齐次坐标，可以得到如下简洁形式：

$$\begin{bmatrix} \boldsymbol{p} \\ 1 \end{bmatrix} =  
      \frac{1}{Z_c} \begin{bmatrix} f_x & 0 & c_x \\ 
                                     0  & f_y & c_y \\
                                     0  & 0   & 1
                    \end{bmatrix} P_c = \frac{1}{Z_c} K P_c $$

可以看到 $K$ 与点 $P_c$ 无关，其只与相机自身相关 (焦距 $f$，像素平面横纵缩放比例 $\alpha, \beta$ 和主点坐标 $c$), 故称 $K$ 为**内参**。

综上，外参将点从世界坐标系转到相机坐标系，内参再将点转到像素平面。外参其随相机位姿变化而变化，内参保持不变。
...
求解相机外参 $T$，则是整个 vlam 的关键，将在后续学习。

### 2.2 镜头畸变

vslam 不使用[针孔相机](#针孔相机)，而是用基于镜头(lens)的相机。在光路中引入透镜之后，会导致[畸变](#畸变原理)——空间中的直线成像后不直了。
畸变有较多类型，最主要的有2个:

- 径向畸变：以镜片中心为圆心，相同半径上的光折射率一致，不同半径上折射率不同。整个畸变沿径向以圆心对称，故称为径向畸变。

  若成像中心膨胀（文献称中心部分的放大率大于边缘部分放大率），则是桶形畸变。反之边缘膨胀，就是枕形畸变。
  桶形畸变常发生在广角镜头，而枕形畸变在长焦镜头中常见。

- 切向畸变：因为 CCD 和光轴没有垂直导致的。

书上给出了消除上述两种畸变的算法，但没有提及名字。搜索了下，应该是 Brown-Conrady model[^3]。

[^3]: 见 [wikipedia-distortion][brown_conrady]. 原始论文，一个是1966年，一个是1919年，有点难以置信。下下来看了下，没找到书上一样的公式。
书上的公式，大概都是后人不断改进的结果吧。 Wikipedia还提到一个简单改进的 division model
比 Brown-Corrady model 更好，看起来它少了 $p_1$, $p_2$ 项。去畸变是一个大的研究Topic，这些公式应该是最基础的吧。

带入公式前，首先要将相机坐标系下的点 $P_c$ 转换到归一化平面上，即 $P_n = \frac{1}{Z_c} P_c$，坐标值除以$Z_c$, 使 $Z$ 轴坐标值为1.
结合前面的针孔模型： 

$$\begin{bmatrix} \boldsymbol{p} \\ 1 \end{bmatrix} =  \frac{1}{Z_c} K P_c  = K \frac{1}{Z_c} P_c = K P_n$$

可以看到这个操作在针孔相机模型中也做了的。得到归一化点 $P_n$ 后，$Z$ 已经为1，我们就只直接在平面上看 $(X_n, Y_n)$ 了。
径向畸变与圆有关，所以先计算点到圆心（即坐标原点、光心）的距离 $R_n = (X_n^2 + Y_n^2)^{0.5}$. 然后有神奇的公式：

1. 径向畸变后的点 $P_d$ 坐标：

   $$
    X_d &= X_n(1 + k_1R_n^2 + k_2R_n^4 + k_3R_n^6) \\
    Y_d &= Y_n(1 + k_1R_n^2 + k_2R_n^4 + k_3R_n^6)
   $$

2. 叠加切向畸变后点 $P_d$ 坐标：

   $$
   X_d &+= 2p_1X_nY_n + p_2(R_n^2 + 2X_n^2) \\
   Y_d &+= p_1(R_n^2 + 2Y_n^2) + 2p_2X_nY_n
   $$

就得到了畸变后的坐标 $P_d$. 它已经在归一化平面上了，参考前面的针孔模型，只需要乘上 $K$ 就可以变成像素坐标了，即

$$ \begin{bmatrix} \boldsymbol{p} \\ 1 \end{bmatrix} = KP_d $$
...
畸变原理，有一章节： 
http://www.shcas.net/jsjyup/pdf/2015/9/%E4%B8%80%E7%A7%8D%E5%A4%A7%E8%A7%86%E5%9C%BA%E9%95%9C%E5%A4%B4%E7%9A%84%E7%95%B8%E5%8F%98%E6%A0%A1%E6%AD%A3%E6%96%B9%E6%B3%95.pdf

在[camera-calibration-undistort][undistort_image]有一个直观解释两种畸变影响的图，总结来说径向畸变让点在径向上有移动
