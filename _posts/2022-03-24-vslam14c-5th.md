...
3. 计算 $P_f = (X_f, Y_f, f)$ 转换到像素平面的坐标 $p = (u, v)$
   ...
   - 像素坐标系的坐标原点 $o$ 在平面左上角，$u$ 轴平行同向于相机坐标系 $X$ 轴， $v$ 轴平行同向于 $Y$ 轴。
    ...
    由上，对称成像平面的原点 $O_f$ 与像素平面主点 $c$ 对应，$P_f$ 横、纵坐标经 $\alpha$, $\beta$ 缩放后落到 $p$ 点， 
...
求解相机的内参，就称为**相机标定**。书上提到了比较著名的张正友标定法，使用打印出的棋盘格就可以标定出内参。
OpenCV 直接有标定的[Tutorial][camera_cali_tutorial], 并附带[示例的棋盘格图片][chess_images]。
