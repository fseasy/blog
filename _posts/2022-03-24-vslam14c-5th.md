...
总结下来，对称成像平面、归一化成像平面，都是为了计算方便而人为定义的平面。在后文总结中，我们并不会用到的归一化成像平面。
...
求解相机外参 $T$，则是整个VSLAM的关键，将在后续学习。

### 2.2 镜头畸变

实际使用的相机基本不使用[针孔成像](#针孔相机)，而是用镜头组。在光路中引入透镜之后，会导致[畸变](#畸变原理)——空间中的直线成像后不直了。
畸变有较多类型，最主要的有2个。

- 径向畸变： 以镜片中心为圆心，相同半径上的光折射率一致，不同半径上折射率不同，在镜片的外围最为明显（因此成像时图片边缘畸变更严重）。
  整个畸变沿径向以圆心对称，故称为径向畸变。

  若中心膨胀（文献称中心部分的放大率大于边缘部分放大率），则是桶形畸变，常见于广角镜头。反之边缘膨胀，就是枕形畸变。
