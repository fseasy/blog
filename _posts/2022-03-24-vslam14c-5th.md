...
2. 计算对称成像平面($z = f$)上，$P_c$ 映射到此平面上的坐标$P_{cf} = (X_{cf}, Y_{cf}, f)$ 【中间计算过程】
   
   $P_{cf}$ 表示相机坐标系下，点 $P_c$ 在对称成像平面上（这里假定成为 focal plane)的映射。
   如上图，因为在同一个坐标系下，且针孔相机模型下光透过小孔沿直线传播，故
   
   a. $\triangle O_cO_{cf}A_{cf}$ 和 $\triangle O_cZ_cA_c$ 相似， $\frac {X_{cf}}{X_c} = \frac{f}{Z_c}$, 则 $X_{cf} = \frac{f}{Z_c} X_c$
   
   b. $\triangle O_cA_{cf}P_{cf}$ 和 $\triangle O_cA_cP_c$ 相似及上述相似关系， $\frac {Y_{cf}} {Y_c} = \frac {\overline{O_cA_{cf}}}{\overline{O_cA_c}} = \frac {f}{Z_c}$, 则 $Y_{cf} = \frac {f}{Z_c} Y_c$
