...
2. 小孔成像: 计算 $P_c = (X_c, Y_c, Z_c)$ 落在对称成像平面($Z = f$)上的点 $P_f = (X_f, Y_f, f)$ 
   ...
   对照上图，因为在同一个坐标系下，且针孔相机模型下光透过小孔沿直线传播，故 $P_f$ 就是点 $P_c$ 与对称成像平面的交点。
   $P_f$ 和 $P_c$ 在一条直线上，故向量 $\vec{OP_f}$ 只需由 $\vec{OP_c}$ 经过简单的常量缩放即可得到。
...
基于小孔成像的[针孔相机](#针孔相机)在现实中并未大量应用，vslam使用的是基于镜头(lens)的相机。引入透镜之后，光路就不是直线了，成像会有[畸变](#畸变原理)。
影响较大的畸变主要的有2种:
...
2. 在归一化平面上，带入畸变公式计算畸变后的点位置 $P_d = (X_d, Y_d, 1)$
   ...
   $$distort: (P_n \| k_1, k_2, k_3, p_1, p_2) \rightarrow P_d 
   \tag{2. 畸变坐标计算}$$
...
\begin{pmatrix} \boldsymbol{p} \\ 1 \end{pmatrix} 
   &= K P_d = K \mathop{distort}(P_n) \\
   &= K \mathop{distort}(\frac{1}{Z_c} P_c) 
\end{split} 
\tag{3. 畸变模型公式}
\end{equation}$$
...
双目相机模型，就是要利用*视差*原理来还原物体的深度的，如下图所示：

<div align="center">
  <img src="/assets/posts/vslam/14-ch5/stereo_deepth.png" alt="stereo deepth"
      width="400">
  <p>图3： 双目相机模型求解物体深度</p>
</div>

两个相机光心相距 $b$, 已知点 $P_c$ 落在两个相机对称成像平面$Z=f$上的点分别为 $P_{fL} = (X_{fL}, Y_{fL}, f)$, 
$P_{fR} = (X_{fR} Y_{fR}, f)$， 求解 $Z_c$ 的值。

根据 $\trangle P_cP{fL}P_{fR}$ 和 $P_cO_LO_R$ 相似，有 

$$\frac{Z_c - f}{Z_c} = \frac{\overline{P_{fL}P_{fR}}}{b} 
                    = \frac{\vert X_{fL} \vert + \vert X_{fR} \vert}{b} 
                    = \frac{X_{fL} - X_{fR}}{b}
$$
