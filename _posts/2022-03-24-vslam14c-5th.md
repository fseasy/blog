...
> 从这是参考 [opencv-calibration][opencv_cali] 中[针孔相机模型图][pinhole_model]，参考书上的符号/过程绘制的图。
相比书上的完整描述，少了映射到归一化平面步骤。
...
2. 计算 $P_c$ 映射到对称成像平面($z = f$)上的坐标 $P_f = (X_f, Y_f, f)$ 【中间计算过程】
   
   $P_{f}$ 表示相机坐标系下，点 $P_c$ 在对称成像平面上（focal plane)的映射。
   如上图，因为在同一个坐标系下，且针孔相机模型下光透过小孔沿直线传播，故
   
   a. $\triangle OO_fA_f$ 和 $\triangle OO_cA_c$ 相似， 则 $\frac {\overline{O_fA_f}}{\overline{O_cA_c}} = \frac{\overline{OO_f}}{\overline{OO_c}}$, 
      其中各线段长度可由对应坐标值表示($\overline{O_fA_f} = X_f$, $\overline{O_cA_c} = X_c$, $OO_f = f$),
      有 $\frac{X_f}{X_c} = \frac{f}{Z_c}$， 则 $X_{f} = \frac{f}{Z_c} X_c$
   
   b. $\triangle OA_{f}P_{f}$ 和 $\triangle OA_cP_c$ 相似及上述相似关系， $\frac {Y_{f}} {Y_c} = \frac {\overline{OA_f}}{\overline{OA_c}} = \frac {f}{Z_c}$, 则 $Y_{f} = \frac {f}{Z_c} Y_c$
   
   简单的相似比，就得到了$P_f$的坐标。可以用矩阵形式把这个计算写到一起： 
   
   $$P_f = \frac{f}{Z_c}\begin{bmatrix} X_c \\ Y_c \\ Z_c \end{bmatrix} = \frac{f}{Z_c} P_c$$ 
   
   非常简单……

3. 计算 $P_f$ 到像素平面的坐标 $p$
   
   $P_c \rightarrow P_f$ 可看做是纯光学过程，而 $P_f \rightarrow p$ 则是光到像素的数字化（模拟量到数值）过程。在仅关注坐标求解的条件下，
   我们需要知道的信息如下：

   a. $P_f$ 坐标的单位是 `mm` 等距离单位，而 $p$ 的单位是像素。
   
   b. 相机空间下的 $P_f$ 变成 像素平面$p$, 是一个采样和量化的过程。量化对应到颜色/亮度的表示，这里我们不关心。
      采样是把连续的光子采样成离散像素点的过程，一般需要CCD元件支持。
      CCD一般是一个矩形，内部由一个个像素单元按行、列排布构成。这就是我们的像素平面了！
      空间中的一个点，落到像素平面上的一个点，横纵坐标的变换，我们称为缩放（注意，像素平面已经没有Z轴信息了）。
      一般地，CDD在横、纵方向的像素个数，和物理的长宽比例不同，故横向、纵向的缩放比例不同！
      缩放因子的单位为 `像素/mm`, `像素/m` 等类似单位。
      
      > 为什么横、纵的缩放比例不同？目前我没有找到确切的原因，猜测有几个可能。 1. 像素，本来就不是方形 2. 像素在横、纵的排列间距不同 3. 受生产工艺的影响，像素无法小到横纵个数比例刚好和物理比例一致。 4. 生产误差（概率不高吧？这大概会引起畸变，但不会引起缩放不一致问题）  
      在[相机标定原理介绍（一）][cali_intro1]中，有一个通过传感器参数计算横纵缩放的例子：NiKon D700相机，传感器尺寸：36.0×23.9 mm，最大分辨率为4256×2832； 则横向缩放比例 $dx = 36.0/4256 = 8.46e^{-3}$，纵向缩放比例 $dy = 23.9/2832 = 8.44e^{-3}$。
      这说明，在设计层面，横纵的缩放比例就不同。（PS： 传感器尺寸，就对应到物理长、宽。这个是我一开始没想到的）


[工业相机标定相关知识整理][industry_cali]
