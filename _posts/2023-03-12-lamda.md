...
(a)模型变大本身可以提高质量，但其在安全性和Groundness方面的改进远远落后于人类的表现。 “质量” 由三个指标共同表示： SSI，即 **sensibleness, speciﬁcity, and interestingness**
...
- 安全性做法：定义了一组说明性的安全目标，试图捕捉模型应在对话中表现出的行为(附录a .1)，并使用一组**人口统计学上不同的众包工作者**来为这些目标标记**多轮对话**中的响应(附录a .2, a .3)。然后，我们使用这些标签微调鉴别器，以检测和删除不安全的响应(第6.1节)。
...
探讨了LaMDA在教育（扮演角色）和内容推荐（音乐推荐家）领域的使用，以了解其潜力和不足。与 GPT-3 中prompt的概念类似，我们将LaMDA设定在特定应用程序的多轮对话中，以使LaMDA适应目标应用。本文进行了实验，以比较仅预训练和微调LaMDA模型在特定应用预处理下的特定应用有用性(即有用和正确的回答)和角色一致性(即智能体话语匹配智能体角色)。我们发现这两种模型都可以很好地适应其预期的应用角色，但微调的LaMDA模型更有帮助。

### 2. 相关工作

语言模型和对话系统：里面提到了用外部搜索引擎来提升groudedness. 对比的方法就有 WebGPT.

对话指标： 自动指标和人评估有差距，还是要有人工的评估。早期人工的评估，是给1个汇总的度量结果。现在是分项来定义。这种方式是它的可调试性。

安全和对话模型安全：训练集数据有偏向性；通过有标注数据来降低毒性

Groundedness指标： 最近的 AIS(**Attributable to Identiﬁed Sources**) 表示了一种更精确的评估模型输出是否和外部世界有关的框架。

### 3. LaMDA 预训练

LaMDA经过预训练，可以预测文本语料库中的下一个token。与之前仅在对话数据上训练的对话模型不同，我们在从公共对话数据和其他公共网络文档创建的数据集上预训练了LaMDA。因此，LaMDA可以在微调之前作为通用语言模型使用。
...
预训练过程： 和 GPT 一样 （从前面数据处理可以看到，用的 BPE，这个和 BERT 的sub-word 就不一样，和 GPT 一样）

解码： uses the same **sample-and-rank strategy as Meena** for decoding. 先解码出 Top 40 个，然后再基于候选句的log-likelihood 和 长度来打分，选出 16 个；

We first sample 16 independent candidate responses using top-k (k = 40) sampling (no temperature). The final output is the highest-scoring candidate, where the score is based on the candidate’s log-likelihood and its length.

### 4. 指标

基础指标，SSI.

安全性： 基于 google 人工智能原则 [https://ai.google/principles/](https://ai.google/principles/) ， 细则在附录里。

Groundedness:

- groundedness = 限定在需要外部世界知识（external world）的回答中； = 能够找到权威来源支持的回答 / 含外部知识的回答
    
- informativeness = 能够找到权威来源支持的回答 / 所有回答 （和 groundedness 分母不同）
    
- citation accuracy = 有URL的回答 / 含外部知识的回答
    

Role-specific metrics: Helpfulness and Role consistency： 这个是应用相关的指标
...
- 人工给 S, S, I 打分：
  ...
  - 每轮对话，需要在 3 个维度上打”是“/否, 即 **sensibleness, speciﬁcity, and interestingness**
        
    - 这 3 个指标是级联关系，且 **sensibleness > speciﬁcity > interestingness；** 前面指标是 ”是“, 才需要评估后面； 否则后面自动是”否“
...
2. 训练集：
  ...
  1. 和 SSI 一样，让众包工作者和 LaMDA 对话，收集了 8K 个对话，供 48K 个回合（每个对话平均 5 - 10 轮）。
    
  2. 然后再找人来评这些数据：作者要求众包按 3 种方式来和模型交互： a. 自然形式 b. 涉及敏感话题的交互 c. 根据安全目标对抗地交互（试图破坏安全目标）；
    
    1. 对一个回复，标注者需要判断是否违背了任何一个安全目标。标注可以是 ”是“， ”否“， ”可能“。
        
    2. 对一个回复最终的标签，要看 3 个标注者的结果：至少 2个是否，才认为回复是安全的，label = 1, 否则就是 0
...
1. 训练集：
  ...
  1. 同 SSI 和 Safety, 让众包和模型对话，收集了 4K 对话，共 40K 轮。这次作者对众包的要求是： 让对话尽量包含对信息的获取。
    
  2. 然后找人来标注：
    
    1. 判断每一轮对话，是否包含对外部信息的论述：
        
      1. 如果它说的不是公开的人，就不是： 比如 “我上周烤了三个蛋糕” 就不含外部信息；而 "*Julius Caesar was born in 100 B*" 是外部信息
            
      2. 常识不是：让标注者判断论述是否是对的； 如果 3 个标注者都认为是对的，那这个就被认为是常识，不算对外部知识的论述。
            
    2. 对每一个需要被检查的回复，作者让标注者记录下搜索的query；然后让标注者改写模型的回复，要求合并来自检索结果的精简信息；如果结果来自网络，要把链接附上表示对原文的引用。
            
2. 测试集：
    
  1. 使用的是 Dinan 的 784 轮对话。包含各种主题。上下文被喂给模型，然后吐出回复。
    
    1. 标注者对每个回复，要求标注出是否包含一些事实声明（是的话，就是groundedness指标的分母）；如果是，这些事实是否可以被通过公开数据来验证（如果是，就是 groundedness 的分子）。
        
    2. 每条回复要 3 个人来标注；最终的 groundedness, informativeness和引用正确性(citation accuracy) 是取多数投票(也就是 >= 2)
            
3. 微调和评估数据，都是 English 的！
...
例子："What’s up? RESPONSE not much."
...
3. 再对质量算一个综合分 s= 3 \* P(sensible) + P(speciﬁc) + P(interesting), 按这个分排序
    
4. 取最高的，作为回复（论文说的是 next response. 应该就是这轮的回复？
    

**Self-Training 流程（仅对生成模型）：**

对预训练数据中随机的的 2.5百万轮对话做预测，选出安全分高的 80万轮对话。然后用这个数据集去训练生成（似乎只训练生成模型，不动判别模型）
...
一种解决方法是增加模型参数来记住更多。但仍存在 时间泛化的问题 (**temporal generalization problem**). 有方法尝试用动态/增量训练架构来解决这个问题，但很难覆盖完全。

本文提出的方法是让模型去学会用外部查询工具。

**The toolset TS：定义一个工具集合**

目前就3个，计算器、翻译系统和检索系统。 输入是字符串，对每个输入，这 3 个工具都会跑一遍，并且都会返回。如果没有结果，就返回为空。

**对话数据收集**： 这块和前面的数据集部分类似（更详细）
...
LaMDA中最大的模型使用1024个TPU-V3芯片和123 TFLOPS/s预训练57.7天，使用GSPMD的FLOPS利用率为56.5%[95]。总FLOPS为56.5% \* 123 TFLOPS/s \* 1024个芯片\* 57.7天= 3.55E+23，高于3.14E+23，对应GPT-3[12]的总FLOPS。

我们数据中心的PUE为1.10，我们在TPUv3上的实验测量的每个加速器的系统平均功率约为289W(借用Meena测量值[108])，这意味着我们模型的总能量成本为57.7天\* 1024个芯片\* 289W \* 1.1 \* 24小时/天= 451 MWh，是GPT-3能量的0.4倍[12,108]。在训练时，我们的能量组合(kg CO2e / kWh)约为0.056，因此LaMDA对最大模型的预训练总碳足迹约为25.2 tCO2e。较小模型的预训练和所有模型的微调的碳足迹约为0.7 tCO2e(见表27)，这使得LaMDA的总足迹约为26 tCO2e。因此，训练LaMDA模型的碳足迹比GPT-3[108]小22.1倍，大约相当于22名乘客往返旧金山和纽约(1.2吨co2e /乘客[108])。LaMDA使用了更多的FLOPS，其能量是GPT-3的0.4倍，但其训练碳足迹明显小于GPT-3，这主要是因为我们的能量组合更加优化(LaMDA: 0.056, GPT-3: 0.429[108])。
