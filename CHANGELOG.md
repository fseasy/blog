...
1. 侧边 Nav 在 `col-md` 下（即 >= medium device, 992 px）总是存在的. Nav 上放置任何浮动于视口的东西。
2. Toc 是否生成，独立与页面状态。只有当 Toc 生成，才考虑渲染方式：
   - content-toc 总是渲染。
   - 侧边栏Toc渲染：
     - 当页面整体宽度 > 1300px 
     - 且滚动高度超过content-toc 时
   - 整个切换是动态控制的（监听窗口 size 变化）
   - content-toc 增加渲染效果： 按 2 栏方式渲染；增加 backgound, border, ::after 等。
3. `BackToTop` 控件总是在 Nav 中渲染：
   - 与 Toc 无关，且不考虑额外的遮挡
   - 当 滚动条 超过一个距离，才显示按钮
   - 底部，参考视口在文档层面的距离，以及 footer 的位置，取二者较小的为参考。
