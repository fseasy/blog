...
由上，就是 GLM 的定义了。推广看起来挺简单，但引入了一个新概念，**指数族分布**， 这也是 GLM 的核心。
...
下面我们通过考虑逻辑回归来和进一步理清 GLM：

1. 逻辑回归要求 $Y$ 服从伯努利分布（0/1），即 
   ...
   令 $\eta = \ln(\frac{\theta}{1 - \theta})$, 则 $\theta = \frac{1}{1 + \exp\lbrace -\eta \rbrace}$，带入有
